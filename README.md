...
我后续需要多次调参，而且还要测试qwen和gpt2两个模型的效果。当前流程反复加载pipeline，需要重复计算，帮我避免重复计算，太浪费时间了。而且怎么监控真正瓶颈：用 nvprof 或 torch.profiler 查看时间主要花在模型前向还是数据加载。简单方法：在循环前后加 time.time() 看每个 batch 耗时。监控瓶颈的原理是什么？为什么这么做？有什么相关知识需要我来了解吗？